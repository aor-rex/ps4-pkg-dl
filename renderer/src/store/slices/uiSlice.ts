...
export const createUiSlice: StateCreator<AppState, [], [], UiSlice> = (set, get) => ({
  ...
  addToast: (type, message) => {
    // Dedupe bursts (e.g. rapid poll failures); cap visible stack at 3.
    // Auto-dismiss lives in ToastContainer (pausable on hover), not here.
    const { toasts } = get();
    ...
    set((state) => ({
      ...
      notificationHistory: [...state.notificationHistory, entry].slice(-50),
